...
const SettingUserInfo = (props) => {
  ...
  const submitHandler = async (e) => {
    ...
    try {
      await axios.put(
        `https://a2z-ecommerce.herokuapp.com/user/${authContext.user._id}`,
        {
          name,
          username,
          address,
    ...
    <form onSubmit={submitHandler} className="bg-gray-700 shadow-md w-2/3 p-8">
      <fieldset>
        ...
        <div className="flex flex-col sm:flex-row items-center justify-between mb-2 w-full">
          <label className="text-gray-200 text-lg font-bold" htmlFor="address">
            Address
          </label>
          <div className="mb-2 w-full sm:w-2/3">
            <textarea
              ...
              type="text"
              required
              className="h-32 shadow appearance-none outline-none rounded w-full py-2 px-3 mb-2
                  bg-gray-900 text-gray-100 leading-tight focus:shadow-outline"
            />
          </div>
        </div>
